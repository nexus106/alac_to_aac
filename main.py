...
def duplicate_folder_with_suffix(src_folder, audio_format) -> str:
    # 複製元のフォルダが存在するか確認
    if not os.path.exists(src_folder):
        raise Exception(f"フォルダ {src_folder} が存在しません")

    # フォルダ名を作成（末尾に変換先フォーマットを追加）
    new_folder: str = src_folder + "_" + audio_format
...
def run_shell_command(xld_binary, file, new_folder, audio_format) -> None:
    command: list[str] = [xld_binary, "-o", new_folder, file, "-f", audio_format]
...
if __name__ == "__main__":
    ...
    args = sys.argv
    xld_binary: str = args[1]  # xldの実行ファイル ./xld
    src_folder: str = args[2]  # 元のフォルダパス test_file
    audio_format: str = args[3]  # 変換先のフォーマット aac
